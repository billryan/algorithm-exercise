...
显然，对于这个问题，`dummy node`是不太适用，我们来探讨下取最值的可能性。从以上代码可知边界处理的关键在于需要判断`root->left`和`root-right`是否为`NULL`, `key`的比较则相对固定，分别为`root->left->val`和`root->right->val`, 由此我们自然可以联想到可以使用`left_val`和`right_val`对左右子树的`key`进行「包装」，在子节点与根节点的`key`进行比较之前对`left_val`和`right_val`赋值即可。
