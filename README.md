...
所以这个文档里对于较复杂的数学公式建议使用 $$\LaTeX $$, 因为托管在gitbook上，所以就用了轻量级的katex插件，没有用重量级的MathJax。行内和行间公式都是 \$\$f(x)\$\$`, 区别在于行间公式 \$\$ 写到下一行行首，而行内公式不能写在行首(废话...)。katex对一些高级的 $$\LaTeX $$ 语法不支持，否则无法编译输出到网站和pdf，尽量用简单的 $$\LaTeX $$语法。
