...
![Linked List Cycle](../images/fast_slow_pointer_linked_list_cycle.png)
...
2. 慢指针初始化为`head`, 快指针初始化为`head`的下一个节点，有助于简化快慢指针间比较时对初始化情况做特殊考虑。因为返回`true`的条件为`slow == fast`, 初始化为相等时会干扰此判断。
