...
1. State: f[i] 从i出发能否到达最终位置
2. Function: $$f[j] = j + A[j] \geq i$$, 状态 $$j$$ 转移到 $$i$$, 置为true
3. Initialization: 第一个为true的元素为A.size() - 1
4. Answer: 递推到第 0 个元素时，若其值为 true返回true
...
这种自顶向下的方法需要使用额外的 $$O(n)$$ 空间，保存小于N-1时的状态。且时间复杂度在恶劣情况下有可能变为 $$1^2 + 2^2 + \cdots + n^2 = O(n^3)$$, 出现 TLE 无法AC的情况
...
针对上述自顶向下可能出现时间复杂度过高的情况，下面使用贪心思想对i进行递推，每次遍历A中的一个元素时更新最远可能到达的元素，最后判断最远可能到达的元素是否大于A.size() - 1
...
首先来看看使用动态规划的解法，由于复杂度较高在A元素较多时会出现TLE，因为时间复杂度接近 $$O(n^3)$$.
...
贪心法可以使用自底向上或者自顶向下，首先看看我最初使用自底向上做的。对A数组遍历，找到最小的下标min_index，并在下一轮中用此min_index替代上一次的last_index, 直至min_index为0，返回最小跳数jumps。以下的实现上有个bug，细心的你能发现吗？
